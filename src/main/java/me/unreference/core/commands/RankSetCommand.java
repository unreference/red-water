...
public class RankSetCommand extends AbstractCommand {
  ...
  public void trigger(CommandSender sender, String[] args) {
    ...
    if (rank == null) {
      sender.sendMessage(
          MessageUtil.getPrefixedMessage(getPrefix(), "Rank not found: &e%s", args[2]));
      return;
    }
    ...
    player.sendMessage(
        MessageUtil.getPrefixedMessage(
            getPrefix(), "Your rank has been updated to &e%s&7.", rank.getId().toUpperCase()));
    sender.sendMessage(
        MessageUtil.getPrefixedMessage(
            getPrefix(),
            "Updated &e%s&7's rank to &e%s&7.",
            player.getName(),
